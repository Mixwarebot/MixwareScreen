...
class MixwareScreenConfig:
    ...
    def get_power_loss_path(self):
        path = self.config.value('folder/power_loss_path')
        if not path:
            path = ""
        return path

    def set_power_loss_path(self, path: str):
        self.set_value('folder/power_loss_path', path)
